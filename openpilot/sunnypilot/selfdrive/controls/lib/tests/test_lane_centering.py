...
class TestLaneCentering(unittest.TestCase):
  def test_hard_gates_are_noop(self):
    for kwargs in [
      ...
      with self.subTest(kwargs=kwargs):
        self.assertEqual(_update(LaneCenteringController(), _model(left=-1.5, right=2.1), **kwargs), 0.0)

  def test_lane_change_is_noop(self):
    self.assertEqual(_update(LaneCenteringController(), _model(left=-1.5, right=2.1, lane_change=1)), 0.0)
  ...
  def test_turn_signal_fades_lane_centering_correction(self):
    ...
    self.assertTrue(0.0 < fading < centered)

    for _ in range(300):
      fading = _update(controller, model, authority=0.0, pause_on_signal=True, turn_signal_active=True)
    self.assertLess(abs(fading), 1e-6)
  ...
  def test_turn_signal_pause_can_be_disabled(self):
    ...
    self.assertAlmostEqual(signaled, output, places=7)
  ...
  def test_offset_is_reduced_in_narrow_lane(self):
    ...
    self.assertTrue(np.isclose(at_safe_limit, above_safe_limit))
  ...
  def test_confident_e2e_path_can_fully_break_in(self):
    ...
    self.assertGreater(lane_authority, 0.0)
    self.assertLess(abs(e2e_authority), 1e-9)
  ...
  def test_confidence_loss_drops_filtered_correction(self):
    ...
    self.assertTrue(0.0 < fading < output)
